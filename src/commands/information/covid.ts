  ...
  .addStringOption((option) =>
    option
      ...
      .setDescriptionLocalizations({
        th: 'ประเทศที่คุณต้องการจะสำรวจโควิด-19 เกี่ยวกับ',
      })
      .setRequired(true),
  );
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (response.status !== 200)
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.covid.country_not_found'),
    );
  ...
  if (!data)
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.covid.backend_issue'),
    );
  ...
  const formattedTime =
    ...
    year +
    ' ' +
    (interaction.client as any).i18n.t('commands.covid.when') +
    ' ' +
    hours +
    ':' +
    minutes.slice(-2);
  ...
  const covidEmbed = new EmbedBuilder()
    ...
    .addFields([
      {
        name: (interaction.client as any).i18n.t('commands.covid.cases_total'),
        value: data.cases.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.cases_today'),
        value: data.todayCases.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.deaths_total'),
        value: data.deaths.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.deaths_today'),
        value: data.todayDeaths.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.recovered'),
        value: data.recovered.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.active'),
        value: data.active.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.critical_stage'),
        value: data.critical.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t(
      ...
      {
        name: (interaction.client as any).i18n.t('commands.covid.tests'),
      ...
      {
        name: (interaction.client as any).i18n.t('commands.covid.population'),
        value: data.population.toLocaleString(),
        inline: true,
      },
      {
        name: (interaction.client as any).i18n.t('commands.covid.one_case_per_people'),
      ...
      {
        name: (interaction.client as any).i18n.t('commands.covid.one_test_per_people'),
    ...
    .setFooter({
      text: (interaction.client as any).i18n
