...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName('query')
      .setDescription('Find information on selected anime, manga or characters.')
      .setDescriptionLocalizations({
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const clientAvatar = (interaction.client as any).user?.avatarURL();
  const animeEmbed = new EmbedBuilder()
    .setColor(Colors.Orange)
    .setTitle(
      (interaction.client as any).i18n.t('commands.anime.anime_manga_or_characters'),
    )
    .setDescription(
      (interaction.client as any).i18n.t('commands.anime.similar_stories', {
        title: inputQuery,
      }),
    )
    .setFooter({
      text: (interaction.client as any).i18n.t('commands.anime.auto_cancel'),
      iconURL: clientAvatar,
    });

  const animeSelect = new StringSelectMenuBuilder()
    .setCustomId('anime-choose')
    .setPlaceholder((interaction.client as any).i18n.t('commands.anime.choose_title'));

  const animeRow = new ActionRowBuilder<any>().addComponents(animeSelect);
  ...
  if (response.status !== 200)
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.anime.data_not_found'),
    );
  ...
  for (let i = 0; i < data.data.length; i++) {
    ...
    animeEmbed.addFields(
      inputSearch === 'characters'
        ...
        : [
            {
              name: `${i + 1}. ${titlesEnJp || (interaction.client as any).i18n.t('commands.anime.undefined')}`,
              value:
                titleEn ||
                (interaction.client as any).i18n.t('commands.anime.undefined'),
            },
          ],
    );
  ...
  const collector = (message as any).createMessageComponentCollector({
    ...
    filter: (inter: any) => inter.user.id === interaction.user.id,
    time: 60_000,
  });
  ...
  collector.on('collect', async (inter: any) => {
    ...
    const animeTrailerButton = new ButtonBuilder()
      .setLabel((inter.client as any).i18n.t('commands.anime.trailer'))
      .setURL(`https://youtu.be/${youtubeVideoId}`)
      .setStyle(ButtonStyle.Link);

    const animeTrailerButtonRow = new ActionRowBuilder<any>().addComponents(
      animeTrailerButton,
    );

    animeEmbed
      .setDescription(
        (inter.client as any).i18n.t('commands.anime.selection_detail', {
      ...
      .setFields(
        inputSearch === 'characters'
          ? [
              {
                name: (inter.client as any).i18n.t('commands.anime.name'),
                value: `[${name}](${link} '${(inter.client as any).i18n.t('commands.anime.learn_more')}')`,
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.synopsis'),
                value: description,
              },
            ]
          : [
              {
                name: (inter.client as any).i18n.t('commands.anime.japan_name'),
                value:
                  titlesEnJp ||
                  (inter.client as any).i18n.t('commands.anime.undefined'),
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.english_name'),
                value: titlesEn
                  ? `[${titlesEn}](${link} '${(inter.client as any).i18n.t('commands.anime.learn_more')}')`
                  : (inter.client as any).i18n.t('commands.anime.undefined'),
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.status'),
                value: status,
                inline: true,
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.age_rating_guide'),
                value:
                  ageRatingGuide ||
                  (inter.client as any).i18n.t('commands.anime.undefined'),
                inline: true,
              ...
              {
                name: (inter.client as any).i18n.t('commands.anime.subtype'),
                value: subtype,
                inline: true,
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.episode_count'),
                value: chapterCount
                  ? String(chapterCount)
                  : String(episodeCount),
                inline: true,
              ...
              {
                name: (inter.client as any).i18n.t('commands.anime.release_date'),
                value:
                  startDate || endDate
                    ? `${startDate} - ${endDate}`
                    : (inter.client as any).i18n.t('commands.anime.in_progress'),
                inline: true,
              },
              {
                name: (inter.client as any).i18n.t('commands.anime.popularity_rank'),
              ...
              {
                name: (inter.client as any).i18n.t('commands.anime.rating_rank'),
                value: ratingRank
                  ? `#${String(ratingRank)}`
                  : (inter.client as any).i18n.t('commands.anime.undefined'),
                inline: true,
