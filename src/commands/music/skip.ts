...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!queue) {
    await interaction.reply(i18n('commands.skip.no_queue'));
    return;
  }

  if (!hasDjPermission(interaction, queue)) {
    await interaction.reply(i18n('commands.skip.not_a_dj'));
    return;
  }

  (client.player as any).skip(interaction);
  await interaction.reply(i18n('commands.skip.skipped'));
}
