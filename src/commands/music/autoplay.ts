...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!queue) {
    await interaction.reply(i18n('commands.autoplay.no_queue'));
    return;
  }

  if (!hasDjPermission(interaction, queue)) {
    await interaction.reply(i18n('commands.autoplay.not_a_dj'));
    return;
  }

  const mode = (client.player as any).toggleAutoplay(interaction);

  await interaction.reply(
    mode ? i18n('commands.autoplay.on') : i18n('commands.autoplay.off'),
  );
}
