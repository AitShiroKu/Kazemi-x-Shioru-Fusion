/**
 * Activities Command
 * Run voice channel activities (YouTube Together, games, etc.)
 * Converted from Shioru/source/commands/fun/activities.js
 */
...
export const data = new SlashCommandBuilder()
  .setName('activities')
  .setDescription('Run activities through audio channel')
  ...
  .addStringOption((option) =>
    option
      .setName('name')
      .setDescription('Name of activity you want to start.')
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const token = (interaction.client as any).configs.token;
  const voiceChannel = (interaction.member as any)?.voice?.channel as VoiceBasedChannel | null;

  if (!inputChannel && !voiceChannel) {
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.activities.user_not_in_channel'),
    );
  }

  const targetChannel = inputChannel || voiceChannel;

  const apps: Record<string, string> = {
    youtube: '880218394199220334',
    youtubedev: '880218832743055411',
    poker: '755827207812677677',
  ...
  const response = await fetch(
    `https://discord.com/api/v10/channels/${targetChannel!.id}/invites`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        max_age: 86400,
        max_uses: 0,
        target_application_id: apps[(inputName || '').toLowerCase()]!,
        target_type: 2,
        temporary: false,
        validate: null,
      }),
  ...
  if (response.status !== 200) {
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.activities.can_not_open', {
  ...
  if (data.code && Number(data.code) === 50013) {
    return await interaction.editReply(
      (interaction.client as any).i18n.t('commands.activities.do_not_have_permission'),
    );
  }

  await interaction.editReply(
    (interaction.client as any).i18n.t('commands.activities.join_via_this_link', {
      code: data.code,
    }),
  );
}
