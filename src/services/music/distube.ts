 ...
 */
export function initializeMusicPlayer(client: Client, config: BotConfig): DisTube {
  if (player) {
    console.log('✅ DisTube Music Player already initialized');
    return player;
  }
  ...
  const options = {
    ...
    customFilters: Array.isArray(config.filters)
      ? (Object.fromEntries(config.filters.map((filter) => [filter, filter])) as any)
