...
export const config: BotConfig = {
  ...
  gemini: {
    ...
    model:
      ...
      configJson.gemini?.model ||
      "gemini-2.0-flash-exp",
    temperature: Number(
      process.env.GEMINI_TEMPERATURE ?? configJson.gemini?.temperature ?? 0.7,
    ),
    maxOutputTokens: Number(
      process.env.GEMINI_MAX_OUTPUT_TOKENS ??
        configJson.gemini?.maxOutputTokens ??
        8192,
    ),
  },
  firebase: {
  ...
  debug: {
    enable: process.env.DEBUG_MODE === "true",
    fileLogging: process.env.DEBUG_FILE_LOGGING !== "false",
    consoleLogging: process.env.DEBUG_CONSOLE_LOGGING !== "false",
    webhookURL:
      process.env.DEBUG_WEBHOOK_URL ||
      configJson.logger?.debug?.webhookURL ||
      "",
  ...
  openWeatherToken:
    process.env.OPEN_WEATHER_TOKEN || configJson.open_weather_token || "",
  translation: {
    baseURL:
      process.env.TRANSLATION_BASE_URL || configJson.translation?.baseURL || "",
  },
...
export const SYSTEM_PROMPT =
  process.env.SYSTEM_PROMPT || configJson.systemMessage?.prompt || "";
export const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
export const GEMINI_MODEL: string =
  process.env.GEMINI_MODEL || config?.gemini?.model || "gemini-2.5-flash";
export const GEMINI_TEMPERATURE: number = Number(
  process.env.GEMINI_TEMPERATURE ?? config?.gemini?.temperature ?? 0.7,
);
export const GEMINI_MAX_OUTPUT_TOKENS: number = Number(
  process.env.GEMINI_MAX_OUTPUT_TOKENS ??
...
if (DEBUG_MODE) {
  ...
  console.log(`   - File logging: ${config.debug.fileLogging}`);
  console.log(`   - Console logging: ${config.debug.consoleLogging}`);
  console.log(`   - Log level: ${config.debug.logLevel}`);
  console.log(
    `   - Webhook logging: ${config.debug.webhookURL ? "enabled" : "disabled"}`,
  );
}
